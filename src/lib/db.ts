...
export interface Product {
  ...
  barcode?: string;
  // Diskon default per produk (di-apply otomatis saat ditambah ke cart)
  defaultDiscountType?: 'percentage' | 'nominal' | null;
...
// Sub-menu / Opsi Produk
export interface ProductOptionGroup {
  id?: number;
  productId: number;        // FK ke products
  name: string;             // Contoh: "Topping", "Level Pedas", "Ukuran"
  isRequired: number;       // 0 = opsional, 1 = wajib pilih (IndexedDB can't index booleans)
  isMultiSelect: number;    // 0 = pilih 1 (radio), 1 = bisa pilih banyak (checkbox)
...
export interface ProductOption {
  ...
  name: string;              // Contoh: "Sosis", "Level 1", "Bakso"
  additionalPrice: number;   // Harga tambahan (0 jika gratis)
  sortOrder: number;
  createdAt: Date;
  isDeleted: number;
  deletedAt: Date | null;
...
export interface TransactionItemOption {
  ...
  optionGroupName: string;    // snapshot nama group saat transaksi
  optionId: number;
  optionName: string;         // snapshot nama opsi saat transaksi
  additionalPrice: number;    // snapshot harga tambahan
}
...
class PosDatabase extends Dexie {
  ...
  transactionItemOptions!: Table<TransactionItemOption>;
  ...
  constructor() {
    ...
    // Version 13 — Inspira POS: Sub-menu produk (option groups & options) + diskon default produk
    // Notes:
    //   * `productOptionGroups` dan `productOptions` adalah tabel BARU untuk sub-menu.
    //   * `transactionItemOptions` menyimpan snapshot opsi yang dipilih per item transaksi.
    //   * Field `defaultDiscountType` & `defaultDiscountValue` di products adalah opsional — undefined = tidak ada diskon.
    //   * Tidak ada back-fill: data lama tidak disentuh, semua field baru bersifat opsional.
